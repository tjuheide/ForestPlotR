...
---
### Setting
Here is an example of a forest plot containing combined results of risk ratios for a comparison of three exposure groups (high dose, low dose, control, say) from a hypothetical study. The data are equivalent to those presented [here](ThreeGroupsV1.qmd) but shaped differently. Some familiarity with the `forestploter` package is assumed. The reader might want to look at the [simple forest plot](OneSimplePlot.qmd) walk through before moving forwards.

### Data

This version of the data is in wide format, meaning each exposure group contributes with a set of columns/variables.
...
tm <- forest_theme(
  ...
  ci_pch = 19,
  ci_lwd = 3,
  ci_col = c("#56B4E9","#001d6d"), # we specify 2 colors, one for each comparison,
  legend_name = "", # title on legend - set to missing
  legend_value = c("Low dose", "High dose"), # labels to be used for the two comparisons
  legend_position = "bottom", # place the legend below the plot
...
```{r, echo = FALSE}
#| label: fig-fpthreevII2
#| fig-cap: |
#|   Forest plot with 3 exposure groups combined in one plot
