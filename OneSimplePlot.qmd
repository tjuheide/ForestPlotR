...
-   `ci_column`: here we specify into which of the columns included in `data` the plot should be inserted.
...
This looks exceptionally stupid. @fig-fp3 holds all the data we want to include but the `smallblank`-columns take up way too much space. However, editing the column headers will help this this. We will edit these by naming the columns (variables) of the data set `results`:
...
Notice how we shamelessly use the same, sometime empty, name for several groups of columns. This is intentional.

If we once again run the `forest` function on this data we get @fig-fp4.
...
In @fig-fp4 we can't tell which columns refer to which exposure group. We can make that clear by first making the plot into an object we can manipulate, specifically we want to add additional headers spanning several columns. We also add a line between data and headers for each exposure group - this is probably not strictly necessary in this situation, but it can be helpful to guide the eye.

```{r}
# make the plot into an object that can be edited using functions from the forestploter package
p <- forest(
  data = results[,c(1,11,2,3,11,4,5,11,6,10)],
  est = results$rr,
  lower = results$lcl,
...
-   finally we specify a theme using `theme_forest` to modify general aspects of the plot.
...
The theme is something that needs to be added when running `forest` meaning this should be done at the top of the script. Here is the entire script necessary to make @fig-fp7:
...
p <- forest(
  ...
  xlim = c(.5, 1.5),
  ticks_at = c(.5, .75, 1, 1.25, 1.5),
  theme = tm # use the customized theme
)
